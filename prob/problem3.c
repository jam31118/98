...
int producer(struct sh_data_t *sh_data_p ,sem_t *empty, sem_t *mutex, sem_t *full){
	int num, idx;
	idx =0;
	num =1;
	do{
		...
		*sh_data_p->write_ptr = num++;
		sh_data_p->write_ptr = (buffer+((idx++)%20));
...
int consumer(struct sh_data_t *sh_data_p, sem_t *empty, sem_t *mutex , sem_t *full) {
	int idx;
	idx = 0;
	do{
		sem_wait(full);
		...
		*sh_data_p->read_ptr = 0;
		sh_data_p->read_ptr = (buffer+((idx++)%20))
